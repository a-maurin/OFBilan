"""Tests des défauts interactifs de période (ask_periode_dept)."""
...
def test_default_date_deb_is_first_day_of_current_year() -> None:
    ...
    with patch("bilans.common.prompt_periode.dt.datetime") as mock_dt:
...
def test_ask_periode_dept_uses_dynamic_defaults_on_empty_input() -> None:
    fixed = datetime(2026, 5, 28, 12, 0, 0)
    inputs = iter(["", "", ""])
    with (
        patch("bilans.common.prompt_periode.dt.datetime") as mock_dt,
        patch.object(pp, "_is_interactive", return_value=True),
        patch("builtins.input", side_effect=lambda _prompt: next(inputs)),
    ):
        mock_dt.now.return_value = fixed
        mock_dt.strptime = datetime.strptime
        deb, fin, dept = pp.ask_periode_dept()
...
def test_ask_periode_dept_non_interactive_requires_dates() -> None:

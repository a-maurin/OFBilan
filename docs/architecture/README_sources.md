...
Ce document décrit les **sources de données**, les **champs** utilisés et les **règles de filtrage** appliquées dans les bilans. Les chargements sont centralisés dans `src/bilans/common/loaders.py`.
...
| -------------------------------- | ------------------------------------------------------------------------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Points de contrôle**           | `data/sources/sig/point_ctrl_YYYYMMDD_wgs84.gpkg`                                                | GPKG          | Contrôles OSCEAN (un fichier par année, suffixe de date le plus récent)                                                                                                         |
| **PEJ**                          | `data/sources/suivi_procedure_judiciaire_YYYYMMDD.ods` ou `suivi_procedure_enq_judiciaire_*.ods` | ODS           | Procédures d’enquête judiciaire                                                                                                                                                 |
| **PA**                           | `data/sources/suivi_procedure_administrative_YYYYMMDD.ods`                                       | ODS           | Procédures administratives                                                                                                                                                      |
| **PVe**                          | `data/sources/Stats_PVe_OFB*.csv` ou `.ods`                                                      | CSV (;) / ODS | Procès-verbaux électroniques OFB (fichier le plus récent par date de modification)                                                                                              |
| **Points infractions PJ**        | `data/sources/sig/points_infractions_pj/localisation_infrac_FAITS_YYYYMMDD.gpkg`                 | GPKG / SHP    | Géolocalisation des faits PJ                                                                                                                                                    |
...
**Source de référence** : `data/sources/sig/points_infractions_pj/` (`localisation_infrac_FAITS_*.gpkg` ou `.shp`, voir `get_points_infrac_pj_path` / `load_points_infrac_pj`).
...
**Ne pas** utiliser `points_infractions_pj` pour les PA. Il suffit de la source **points de contrôle OSCEAN** (`data/sources/sig/points_de_ctrl_OSCEAN_*` / chargement via `load_point_ctrl`).
...
- **Périmètre PNF (communes)** : en priorité le shapefile `**ref/sig/communes_pnf/communes_pnf.shp`** (polygones des communes du parc ; colonne INSEE attendue : `INSEE_COM`, `CODE_INSEE`, `insee_comm`, etc.). À défaut, fichier `**communes_PNF.csv`** dans `ref/` ou `data/sources/` (`NOM`, `CODE_INSEE`, `Adhesion`, `Coeur`, `RI`, …). Le chargeur `load_pnf` dans `src/bilans/common/loaders.py` unifie ces sources.
...
Le script `scripts/inventaire/inventaire_donnees.py` produit les **valeurs distinctes et effectifs** des champs clés (theme, type_actio, domaine, nom_dossie, NATINF, etc.) sans modifier les bilans. Sortie : `data/out/inventaire/` (CSV + résumé texte). Utiliser ces résultats pour valider les thèmes et NATINF avant d’ajouter de nouveaux objets d’analyse.
...
- **Sorties :** `data/out/inventaire/*.csv` (un CSV par source et champ) + `inventaire_resume.txt`. Jointure optionnelle avec `ref/liste_natinf.csv` pour libeller les NATINF.
...
- **Sorties :** `data/out/bilan_natinf/` : `pve_par_natinf.csv`, `pve_par_natinf_zone.csv`, `pej_par_natinf.csv`, `pej_par_natinf_clotur.csv`, `pej_par_natinf_suite.csv`, `pej_par_natinf_theme.csv`.
...
- **Sorties :** `data/out/bilan_temporel/` : `controles_par_mois.csv`, `controles_par_trimestre.csv`, `pve_par_mois.csv`, `pve_par_trimestre.csv`, `pej_par_mois.csv`, `pej_par_trimestre.csv`.
...
- **Sorties :** `data/out/bilan_procedures/` : `pej_duree_resume.csv`, `pej_clotur_global.csv`, `pej_suite_global.csv`, `pej_duree_par_theme.csv`, `pej_duree_par_domaine.csv`.

### 6.5 Parties communes réutilisables

- **Loaders :** `load_tub_pnf_codes(root)` dans `src/bilans/common/loaders.py` retourne `(tub_codes, pnf_codes)` pour les agrégations par zone (Département, TUB, PNF) sans dupliquer le chargement des référentiels.
...
| ------------------------------------------------------------------------- | --------------------------- | ------------------------------------------------------------------------------------------------ |
| `load_point_ctrl`                                                         | `src/bilans/common/loaders.py` | Charge les points de contrôle (optionnel : dept, date_deb, date_fin)                             |
